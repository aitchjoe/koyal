...
- 企业内通常会限制了互联网访问，并不能直接下载制品及制品依赖，因此不止容器、任何一个技术栈的 Hello World 在企业内部都不能简单跑起来，以至于我们有一个 [Hello Hard World](openshift-start.md) 的专题。
...
- 在中期关注到这个问题的是 Bitnami，和 Docker 公司一样，Bitnami 制作了大量主流软件的 [Non-Root](https://docs.bitnami.com/kubernetes/faq/configuration/use-non-root/) 镜像，虽然影响力没有 Docker 大但值得尝试。
  - Bitnami 本质上也是做 [chmod](https://github.com/bitnami/containers/blob/main/bitnami/nginx/1.29/debian-12/Dockerfile#L46) 等等，但肯定比我们更专业更周全，作为业务应用的开发者完全没必要在这上面耗时间精力，用现成的就好。
...
- 请用户**尽量使用**同一产品的 Unprivileged 镜像、尽量避免 `chmod 777` 方式；但是业界并没有一个类似 `nginx-unprivileged` 的镜像命名规范，仍需要用户具体情况具体分析。
  - 作为我们最主要的技术栈，[Java](container-public-image.md#java) 镜像基本都没问题。
...
- 和其他所有制品相比，容器制品是构建过程最复杂、对构建环境要求最高的。一个重点是因为 Dockerfile 里的 [RUN](https://docs.docker.com/engine/reference/builder/#run) 指令可以"execute any commands"，而该命令所产生的后果，如创建新文件、修改系统配置等，很难在不真正执行该命令时就做出精准预期。
  - 因此构建容器镜像时，需要实际启动一个容器并在其中运行所有的 `RUN` 命令，而在 [CI](cicd-tldr.md) 环境会更复杂，因为现代的 CI Job 很多也是以容器运行，那么就得在容器中构建容器镜像，也就是所谓的 Docker in Docker（DinD），在性能或安全方面都有影响。
...
- 具体构建指令的说明参见 [Dockerfile 解析](dockerfile-walkthrough.md)。
