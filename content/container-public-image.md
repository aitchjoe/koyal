...
- 关于其他技术栈如 Python 等的镜像选择，我们作为 PaaS 平台方主要是从容器角度给出建议和提醒，比如倾向用哪个仓库、什么是可信镜像等等；而特定技术栈的容器镜像的确定，由该领域专家调研，并反馈补充到我们文档。
...
- Docker 官方镜像（前缀 `docker.io/library`）：AWS 的一个亮点就是提供 [Docker official](https://gallery.ecr.aws/docker/) 镜像的拷贝，基本可以说有一个 `docker.io/library/xxx` 就存在一个 `public.ecr.aws/docker/library/xxx`，即使软件官方文档未提及；如果是这种情况，**强烈建议**尽快转到 AWS 仓库。
  - 由于 AWS 晚于 Docker，未考虑将之前（或者太早）的镜像版本转过来，比如 [nginx](https://gallery.ecr.aws/docker/library/nginx) 就没有 `1.18` 之前的版本，当然这也不影响现在的使用。
- Docker 非官方镜像：
  1. [Bitnami](https://gallery.ecr.aws/bitnami/) 制作了大量主流软件的 [Non-Root](https://docs.bitnami.com/kubernetes/faq/configuration/use-non-root/) 镜像，因此可以先确认 Bitnami 是否提供了想要使用软件的镜像。
     ...
     - 但用 Bitnami 也有好处，无论什么软件，Bitnami 在文件路径、配置等方面都保持了一致的风格，如果用了一个 Bitnami 镜像，再用另一个完全不同种类软件的 Bitnami 镜像，上手也会更快。
...
- 操作系统：如上所述，一个产品的容器镜像有基于 Debian 或 Ubuntu 但很少两者都有，因此这里的选择聚焦在选主流操作系统的精简版还是原生精简的 Alpine？
  ...
  - 由于精简镜像的一个问题是缺乏常见工具用于 Debug，因此我们粗略比较一下三个系统的默认工具，并加上对比的非 Slim 镜像（以下镜像前缀均为 `public.ecr.aws/docker/library/`）：
    | 容器镜像                        | 体积  | bash | curl | ip | lsof |nc | netstat | nslookup | ping | ps | sed | ss | telnet | top | traceroute | vi | wget |
    | ------                          | ----- | ---- | ---- | -- | ---- |-- | ------- | -------- | ---- | -- | --- | -- | ------ | --- | ---------- | -- | ---- |
    | `alpine:3.22.1`                 | 4.13M |   ×  |   ×  |  √ |   √  | √ |     √   |     √    |   √  |  √ |  √  |  × |    ×   |  √  |      √     |  √ |   √  |
    | `ubuntu:noble-20250716`         | 34.3M |   √  |   ×  |  × |   ×  | × |     ×   |     ×    |   ×  |  √ |  √  |  × |    ×   |  √  |      ×     |  × |   ×  |
    | `debian:bookworm-20250811-slim` | 32.1M |   √  |   ×  |  × |   ×  | × |     ×   |     ×    |   ×  |  × |  √  |  × |    ×   |  ×  |      ×     |  × |   ×  |
    | `debian:bookworm-20250811`      | 52.3M |   √  |   ×  |  × |   ×  | × |     ×   |     ×    |   ×  |  × |  √  |  × |    ×   |  ×  |      ×     |  × |   ×  |
  - 对比以上各镜像所带的工具，可以看出 Debian 的 Slim 和非 Slim 镜像虽然体积不同、但常用工具并无二致，根据 Debian 的 [How It's Made](https://github.com/docker-library/docs/blob/master/debian/README.md#how-its-made) 提示，确认 Slim 实际 [Exclude](https://github.com/debuerreotype/debuerreotype/blob/master/scripts/.slimify-excludes) 的内容主要是文档、多语言支持等；当然，和基本具备以上所有工具的 VM 操作系统比，无论 Slim 还是非 Slim 的容器镜像都算精简的了。
  ...
  - 虽然个人倾向于体积更小且带有更多工具的 Alpine 镜像，但用户可以综合以上因素自行选择，尽量**保持一致**就行。
