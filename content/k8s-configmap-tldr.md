...
- Pod Manifest 解读参见 [Pod TLDR](k8s-pod-tldr.md)，此处只关注 Pod 对 ConfigMap 的引用。
...
- 将 ConfigMap 的配置数据挂载为容器中的文件在语法上更复杂，也需要使用 Kubernetes 的存储机制，也就是说会把 ConfigMap 当成一种存储的来源，详细说明参见 [K8s Debug](k8s-debug.md#configmap) 的 ConfigMap 一节。
