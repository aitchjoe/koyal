...
部分应用会有多个实例通过 NFS 等共享网络文件服务，虽然 Kubernetes 从技术上来讲也支持挂载 NFS，但我们的 Kubernetes 平台定位是无状态应用，因此不会提供存储高可用的保证；但即使提供，那主要也是给数据库、缓存、消息队列等需要持久存储的中间件使用的，普通业务应用应该将共享文件存储看作类似数据库的[后端服务](cn-12factor.md#后端服务)；而对象存储正是这个领域的主流解决方案，早已不是新技术了。
...
参见 [Helm TLDR](helm-tldr.md)，和以上 Dockerfile 类似，Helm 不是部署 Kubernetes 应用的唯一方式。
