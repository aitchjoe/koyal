...
- 还有一种貌似原始、但对于简单场景也很实用的做法，就是文本作图，这个可以自己手工画，但也有 [ASCIIFlow](https://asciiflow.com) 这样的工具帮助完成。以下是我们在[*真实文档*](why-here.md)中的图例：
  ...
  +-------------+                         +-------------------------+
  ```
- Unicode 有[图形文字](https://www.unicode.org/emoji/charts/full-emoji-list.html)，现代的浏览器和编辑器大多支持，当然这只能表示极简单的含义，但可以给页面点缀一些活跃的效果💡
- 事实上业界技术文档使用图片已经**非常节制**了，主要是极少量的架构图或流程图，如 [Kubernetes](https://github.com/kubernetes/website/tree/release-1.33/content/en/docs/images)、[Spring](https://github.com/spring-projects/spring-framework/tree/v6.2.10/framework-docs/modules/ROOT/assets/images)、[GitLab](https://gitlab.com/gitlab-org/gitlab/-/tree/v18.1.5-ee/doc)，反观我们大量使用的是操作示例的截图。
  - 当然也有 [OpenShift](https://github.com/openshift/openshift-docs/tree/enterprise-4.18/images) 等文档使用了大量图片，我们也承认这对读者更加友好；但是对于作者，截图特别是重现复杂步骤且保持一致性的截图，这个的工作量比想象中大很多，再考虑考虑比如升级后界面又有了调整、哪怕仅仅是布局方面的？如果读者仍不理解，仍坚持"我不管，我就要直观的截图"，那么请看下一条。
  - 以我的实际观察，至少在我们的文档，操作截图的更新频率要**远低于**文字。
- 总之使用 Markdown 在图像方面确实不够方便直观，但请多考虑一下现实、多考虑一下是否真的那么必须？
