...
- 我们实际使用的本地部署产品是 [OpenShift Container Platform](https://www.redhat.com/en/technologies/cloud-computing/openshift/container-platform)，通常简称为 **OCP**；而 OCP 产品我们跨越了两个大版本，早期是 OCP 3 但现在已全部转到 OCP 4。
- 从 2018 年开始搭建 PaaS 平台，到 2025 年已发展为多个 [*OCP 集群*](why-here.md)，现在更适合的应该是 [OpenShift Platform Plus](https://www.redhat.com/en/technologies/cloud-computing/openshift/platform-plus) 这个产品，因为支持多集群管理及安全等，这是"[most complete solution](https://www.redhat.com/en/technologies/cloud-computing/openshift/self-managed)"。
...
- 还有两个非平台用途的产品：
  - 运行在个人电脑的 [OpenShift Local](https://developers.redhat.com/products/openshift-local/)，类似 [minikube](https://minikube.sigs.k8s.io/) 用于学习或开发用途，前身是 CodeReady Containers（CRC）和 minishift。
  - 用于边缘计算的 [MicroShift](https://www.redhat.com/en/topics/edge-computing/microshift)，应该不算 OpenShift 系列，只是"Derived from OpenShift"并做了裁剪，参见 [Key differences from OpenShift Kubernetes Engine](https://docs.redhat.com/en/documentation/red_hat_build_of_microshift/4.19/html/understanding_microshift/microshift-understanding#microshift-differences-oke_microshift-understanding)；另外由于足够轻量也可以用于开发用途，参见 [Why developers should use MicroShift](https://developers.redhat.com/articles/2025/02/20/why-developers-should-use-microshift)。
...
- Project：和 Kubernetes [Namespace](k8s-namespace-tldr.md) 类似。对比 OpenShift 关于 [Project](https://docs.redhat.com/en/documentation/openshift_container_platform/4.18/html/project_apis/project-apis) 和 [Namespace](https://docs.redhat.com/en/documentation/openshift_container_platform/4.18/html/metadata_apis/namespace-v1) 的文档，是前者用于成员、配额等管理控制功能，但两者的主要目标也都是为了 [isolation](https://kubernetes.io/docs/concepts/overview/working-with-objects/namespaces/)。
  - 而在 API 层面，无论 OCP 3 或者 4，Namespace（[3](https://github.com/openshift/openshift-docs/blob/enterprise-3.11/rest_api/core/namespace-core-v1.adoc) / [4](https://docs.redhat.com/en/documentation/openshift_container_platform/4.18/html/metadata_apis/namespace-v1)）和 Project（[3](https://github.com/openshift/openshift-docs/blob/enterprise-3.11/rest_api/project_openshift_io/project-project-openshift-io-v1.adoc) / [4](https://docs.redhat.com/en/documentation/openshift_container_platform/4.18/html/project_apis/project-project-openshift-io-v1)）的 Specification 几乎完全相同；也可以对比 `oc get namespace/xxx -o yaml` 和 `oc get project/xxx -o yaml` 的实际结果，除了 `Kind` 外，包括 Annotation 等元数据、甚至连 `uid` 都是相同的。
  - Project 和 Namespace 是一一对应的，创建一个会自动生成另一个；除了兼容考虑，不觉得有什么必要还保留 Project。
...
- oc 即 [OpenShift CLI](https://docs.redhat.com/en/documentation/openshift_container_platform/4.18/pdf/cli_tools/openshift-cli-oc#cli-tools-list)，但运行 `oc --help` 显示的是 OpenShift Client，无论哪种说法都很贴切。
- CLI 应该是 IaaS / PaaS 或者云平台的标配，比如 [AWS CLI](https://aws.amazon.com/cli/)、[Azure CLI](https://learn.microsoft.com/en-us/cli/azure/)、[阿里云CLI](https://help.aliyun.com/zh/cli/) 等等等等。
  - 虽然 OpenShift 也有提供 [Web Console](https://docs.redhat.com/en/documentation/openshift_container_platform/4.18/html/web_console/web-console-overview) 且对普通用户更友好，但即使不论 GUI Vs. CLI 的使用场景，实际也是 oc 覆盖的功能更完整。
- 在 [Usage of oc and kubectl commands](https://docs.redhat.com/en/documentation/openshift_container_platform/4.18/html/cli_tools/openshift-cli-oc#usage-oc-kubectl) 解释了和 kubectl 的关系：
  > The oc binary offers the **same capabilities** as the kubectl binary, but it **extends** to natively support additional OpenShift Container Platform features
  - 也就是说在网上文档看到的 `kubectl ...` 命令示例，基本都可以无缝替换为 `oc ...` 执行。
- 现在 PaaS 平台有多个 OpenShift 集群且版本不一致，因此注意使用对应最高版本的客户端；一般高版本客户端对低版本服务端基本都没问题，即使是大版本差别，比如 oc 4 访问 OCP 3。
- 普通用户日常使用的 oc 命令，在 [OpenShift 实操入门](openshift-start.md#hello-oc) 中有实际示例：
  - `oc --help` / `oc options`：帮助 / 列出全局选项，`oc <command> --help` 则是查看各命令详细说明。
  - `oc login` / `oc logout`：登录指定的 OpenShift 集群 / 退出当前集群。
  - `oc projects` / `oc project`：列出有权限访问的所有 Project / 显示当前 Project 或切换 Project。
  - `oc get`：在当前或指定 Project 下，列出某类 Kubernetes Object 或显示某个 Object 的详细信息，类似 `oc get pods` 或 `oc get pod/<pod-name> -o yaml`。
  - `oc apply` / `oc delete`：新建或变更 / 删除某个或多个 Kubernetes Object。
  - `oc logs`：查看容器日志。
  - `oc rsh`：SSH 进入 容器。
