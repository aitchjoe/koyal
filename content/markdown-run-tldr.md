...
- 除了内容上区分结构和层次，通过网站呈现的标题还带有导航性质，一是文内导航的标题锚点，二是自动生成 TOC（Table of Contents）目录。
  - 但并不是所有网站都支持这种功能，大家可以对比这个同一文档在 [*GitLab*](why-here.md) 和[*飞书*](why-here.md)（包括网页版和 APP）的效果。
...
- **知识一定要串联，多用链接。**
- 引用地址尽量带上**版本信息**，如 [github.com/.../**v1.23.17**/.../sync.go](https://github.com/kubernetes/kubernetes/blob/v1.23.17/pkg/controller/deployment/sync.go#L223) 或 [docs.spring.io/.../**3.4**/.../efficient-images.html](https://docs.spring.io/spring-boot/3.4/reference/packaging/container-images/efficient-images.html#packaging.container-images.efficient-images.layering)，否则过一段时间后可能指向的就不是你期望的内容了。
  - 源码使用正式版的 Tag，否则也可能被移除。
  - 文档的情况比较复杂：
    - 文档通常只对应到软件的中版本，如 Kubernetes、Grafana 等，Spring 之前有具体到小版本但现在也是到中版本，而 Vue.js 只有大版本的文档，显然版本越具体会变动的可能性越小。
    - 产品文档网站如 Spring、Grafana 会保留所有版本，但 Kubernetes 会移除较旧的版本；对于后者，只能说基本不会变的内容不带版本也行，否则仍倾向于带版本号，或者使用以下的文档源码方式。
    - 最新文档的 URL 通常不包含版本信息，可以先转到旧版本文档获取 URL，然后手工替换为新版本号。
    - 开源软件的文档通常也作为源码管理，而源码的正式 Tag 是不会被移除的，也就是说文档也可以引用到源码地址。
